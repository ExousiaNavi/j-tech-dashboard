...
export default function Dashboard() {
  ...
  const [recentlyCreatedOrgs, setRecentlyCreatedOrgs] = useState<Set<string>>(new Set());
  ...
  const dynamicOrgs = useMemo(() => {
    const orgsFromClients = Object.values(clients).map((c) => c.org_id || "Unassigned");
  ...
  const handleCreateOrg = async () => {
    ...
    if (dynamicOrgs.includes(trimmedName)) {
    ...
    try {
      ...
      if (created) {
        ...
        setSuccessMessage(`Organization "${trimmedName}" created successfully!`);
        ...
        setErrorMessage(`Organization "${trimmedName}" already exists in the system!`);
      }
    } catch (error) {
      ...
      setErrorMessage(`Error creating organization: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setIsCreating(false);
    }
  };
  ...
  // Stats
  const totalPCs = Object.keys(clients).length;
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-3 text-xs">
                  ...
                  <span className="text-green-400">{totalPCs} online</span>
                  <span className="w-1 h-1 bg-gray-600 rounded-full"></span>
                  <span className="text-gray-400">{dynamicOrgs.length} orgs</span>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
              <div className="w-full lg:w-auto">
                <span className="text-base text-gray-400 font-medium whitespace-nowrap">Organization:</span>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-1.5 flex-1 min-w-0">
                          ...
                          {isRecentlyCreated && (
                            <span className="w-1.5 h-1.5 bg-red-500 rounded-full animate-pulse" title="New"></span>
                          )}
                        </div>
                  ...
                  {dynamicOrgs.length > 3 && (
                    ...
                    >
                      ...
                      {dynamicOrgs.slice(3).map((org) => {
                        ...
                        const isRecentlyCreated = recentlyCreatedOrgs.has(org);
                        return (
                          <option key={org} value={org} className={isRecentlyCreated ? "bg-red-900/20" : ""}>
              ...
              <div className="flex items-center gap-2 w-full lg:w-auto">
                <div className="relative flex items-center w-full lg:w-[200px]">
                  ...
                  <input
                    ...
                    onKeyPress={(e) => e.key === 'Enter' && !isCreating && handleCreateOrg()}
            ...
            {recentlyCreatedOrgs.size > 0 && (
              <div className="flex items-center gap-2 text-xs text-amber-300 bg-amber-900/20 border border-amber-800/30 rounded-lg px-3 py-2">
                ...
                <span>
                  New organizations: {Array.from(recentlyCreatedOrgs).join(", ")}. 
                  Device count updates when PCs are assigned to them.
                </span>
              </div>
            )}

            {/* Active Filters */}
            {(searchQuery || (orgFilter && dynamicOrgs.length > 0 && orgFilter !== dynamicOrgs[0])) && (
              <div className="flex items-center gap-2">
                <span className="text-xs text-gray-500">Active filters:</span>
                <div className="flex flex-wrap gap-1.5">
                  {orgFilter && dynamicOrgs.length > 0 && orgFilter !== dynamicOrgs[0] && (
      ...
      {displayClients.length > 0 ? (
        ...
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-semibold text-white mb-2">
            {searchQuery || orgFilter !== "" ? "No Matching Devices" : "No Connected Devices"}
          </h3>
